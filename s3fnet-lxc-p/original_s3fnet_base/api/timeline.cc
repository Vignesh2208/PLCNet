...
void* Timeline::thread_function() {
	...
	while(1) {
		...
		do {
			...
			//**** I think this is the start of a synchronization window.
			...
			//
			change_state(BLOCKED); //**** Change state of Timeline to BLOCKED
			nxt_time = next_time();//**** Returns the time of the last-time event in the Timeline's event list
			...
			if( __interface_control->get_numTimelines() > 1 ) {
				...
				if( nxt_time < 0 || __min_sync_cross_timeline_delay < 0  )  {
					ltime_t this_window = MAX(__time, nxt_time) + __window_size; //*** The next synchronization window could be equal to the window size
					if(this_window > __time)
					{
#ifdef MUTEX_BARRIER
						__interface_control->bottom_barrier.wait( this_window );//*** refer to barrier_mutex_t documentation. The minium of all offered times is chosen
#endif
#ifdef SCHED_YIELD_BARRIER
