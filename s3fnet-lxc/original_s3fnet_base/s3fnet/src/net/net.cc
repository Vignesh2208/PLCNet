...
Net::~Net()
{
  ...
  if(netacc) delete netacc; // Net accessory, some extra options for the net


  //nets - list of subnetworks - a map from id to pointer to network
  ...
  //hosts  map of hosts - id to pointer to host
...
void Net::config(s3f::dml::Configuration* cfg)
{
  ...
  else // not top net , a subnet
  {
    nhi = myParent->nhi; // Probably set in call to DmlObject in constructor
    nhi += id;
  }
