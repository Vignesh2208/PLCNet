 ...
 * \file RngStream.cc
 *
 * \brief Source file of RngStream class, %Random number stream generator.
 *
 */
...
//
double RNGS::RngStream::nextSeed[6] =
{
...
RNGS::RngStream::RngStream () 
{
   ...
   // since nextSeed is shared
   pthread_mutex_lock(&nextState_lock);
   MatVecModM (A1p127, nextSeed, nextSeed, m1);
   MatVecModM (A2p127, &nextSeed[3], &nextSeed[3], m2);
   pthread_mutex_unlock(&nextState_lock);
}
...
//
void RNGS::RngStream::ResetStartStream ()
{
   for (int i = 0; i < 6; ++i)
      Cg[i] = Bg[i] = Ig[i];
}
...
//
void RNGS::RngStream::ResetStartSubstream ()
{
   for (int i = 0; i < 6; ++i)
      Cg[i] = Bg[i];
}
...
//
void RNGS::RngStream::ResetNextSubstream ()
{
...
//-------------------------------------------------------------------------
bool RNGS::RngStream::SetPackageSeed (const unsigned long seed[6])
{
   if (CheckSeed (seed))
...
//-------------------------------------------------------------------------
bool RNGS::RngStream::SetSeed (const unsigned long seed[6])
{
   if (CheckSeed (seed))
      return false;                   // FAILURE     
   for (int i = 0; i < 6; ++i)
...
//
void RNGS::RngStream::AdvanceState (long e, long c)
{
...
//-------------------------------------------------------------------------
void RNGS::RngStream::GetState (unsigned long seed[6]) const
{
   for (int i = 0; i < 6; ++i)
...
//-------------------------------------------------------------------------
void RNGS::RngStream::WriteState () const
{
...
//-------------------------------------------------------------------------
void RNGS::RngStream::WriteStateFull () const
{
    int i;
...
//-------------------------------------------------------------------------
void RNGS::RngStream::IncreasedPrecis (bool incp)
{
   incPrec = incp;
}
...
//-------------------------------------------------------------------------
void RNGS::RngStream::SetAntithetic (bool a)
{
   anti = a;
}
...
//
int RNGS::RngStream::RandInt (int low, int high)
{
    return low + static_cast<int> ((high - low + 1.0) * RandU01 ());
};

pthread_mutex_t RNGS::RngStream::nextState_lock;
