...
class BlockInterface(object):
	...
	# ** field is of type BlockInterfaceField
	# ** add field argument to fieldNameMap with the field name as the key
	...
	# -- add field to the passed struct (AwlStruct) class.
	def __buildField(self, cpu, struct, field, isFirst):
		if isFirst:
			# Align to word boundary.
			# defined in datastructure.py
			# ** ultimately creates a field object and adds it to a list of fields of the struct
	...
	# -- build each IN, OUt,INOUT and STAT fields and add it to self.struct (AwlStruct class object)
	...
	def buildDataStructure(self, cpu):
		...
		if self.hasInstanceDB:
			...
			## ** create a AwlStruct and call build fields for all IN, OUT, INOUT and STAT fields
		...
		# ** note a separate AwlStruct for all TEMP variables/fields
	...
	def __getOperatorForField_TEMP(self, identChain, wantPointer):
		...
		if wantPointer:
			ptrValue = structField.offset.toPointerValue()	#** in offset.py - some shift operation on byte offset and bit offset values.
			ptrValue |= AwlIndirectOp.AREA_L		# ** AREA_L for Local Stack
			return AwlOperator(type = AwlOperator.IMM_PTR,
					   width = 32,
					   value = Pointer(ptrValue))  # ** Pointer class defined in datatypes.py
		oper = AwlOperator(type=AwlOperator.MEM_L,	       # ** MEM_L refers to LOCAL STACK Memory
	...
	def getOperatorForField(self, identChain, wantPointer):
		...
		#  ** getField defined in datastructure.py
		...
		if wantPointer:
			...
			ptrValue |= AwlIndirectOp.AREA_DI		# ** AREA_DI - For Instance Data Block
			return AwlOperator(type = AwlOperator.IMM_PTR,
					   width = 32,
		...
		if structField.dataType.type in BlockInterface.callByRef_Types:
			...
			# ** Memory-Indirect Access - operators.py - resolve function
