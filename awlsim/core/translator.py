...
class AwlTranslator(object):
	...
	# -- extract instruction object from raw instruction. Set the instruciton pointer.
	def __translateInsn(self, rawInsn, ip):
		ex = None
		try:
			insn = AwlInsnTranslator.fromRawInsn(self.cpu, rawInsn) # -- refer instrans.py
			insn.setIP(ip)
	...
	# -- extract instruction objs for all raw instructions
	...
	# -- returns a BlockInterfaaceField Object given the rawField by first extracting the field name and datatype.
	def __translateInterfaceField(self, rawField):
		name, dataType, initBytes = self.rawFieldTranslate(rawField) # -- see description
	...
	# initBytes: The initialization bytes for this field, or None.
		...
		field = BlockInterfaceField(name = name,
					    dataType = dataType) # -- refer blocks.py
		return field
	...
	# -- creates a block obj from block class specified. Block class can be FB, OB, UDT ...
	# -- for each type of rawfield (in, out, inout, static .. ) it creates a BlockInterfaceField Object (by calling __translateInterfaceField).
	# -- appends BlockInterfaceField Object to the block objects ( to its interface)
	# -- returns block object created from the block class.
			...
			if dataType.type in {AwlDataType.TYPE_ARRAY,
				...
				if isLastElement and\
				   ...
				   not allowWholeArrayAccess:
					print("Chain = ", chain,chain[-1].indices)
