...
class AwlIndirectOp(AwlOperator):
	...
	def resolve(self, store=True):
		...
		try:
			# ** get the operator type from the pointer area code
			if store:
		...
		if bitwiseDirectOffset:
			...
			directOffset = AwlOffset.fromPointerValue(pointer)	# Create an AWL Offset object from the passed pointer value
		else:
